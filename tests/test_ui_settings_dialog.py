...
class TestSettingsDialog(SettingsDialogTestCase):
    def test_dialog_is_pre_filled_from_current_settings(self):
        settings_store.save_settings({"scan_enabled": False, "scan_interval_minutes": 30, "restore_on_startup": True})
        dialog = self._dialog()
    ...
    def test_save_produces_expected_result_shape(self):
        ...
        self.assertEqual(
            dialog.result,
            {"scan_enabled": True, "scan_interval_minutes": 15, "restore_on_startup": True},
        )
